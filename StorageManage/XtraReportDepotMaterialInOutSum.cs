...
namespace StorageManage
{
    ...
    public partial class XtraReportDepotMaterialInOutSum : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public XtraReportDepotMaterialInOutSum(DataTable dtl ,string BillDate, string Depot, string StorageTypeGuid,string MaterialName,string Spec,string producttype)
        {
            InitializeComponent();
            IniControl_CN();

            DataBind(dtl, BillDate, Depot, StorageTypeGuid,  MaterialName, Spec, producttype);

        }

        private void DataBind(DataTable dtl, string BillDate, string Depot, string StorageTypeGuid,  string MaterialName, string Spec, string producttype)
        {
            xrTableCell1.Text = Depot;//仓库
            xrTableCell3.Text = BillDate;//日期
            //xrTableCell4.Text = BarNo;//助查码
            xrTableCell5.Text = MaterialName;//品名
            xrTableCell6.Text = Spec;//型号
