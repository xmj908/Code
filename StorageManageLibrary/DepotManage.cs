    ...
    public class DepotManage
    {
       ...
       /// <returns></returns>
        public DataTable GetDepotData()
        {
            ...
            try
            {
                string ps_Sql = "select DepotGuid,DepotName as 库位,DepotPerson as  负责人,Telephone as 电话,DepotCoordinate as 库位坐标,Remark as  备注 from Depot order by cast(DepotName as numeric) ";
        ...
        public DataTable GetDepotData2()
        {
            ...
            try
            {
                string ps_Sql = "select DepotGuid,DepotName as 库位,DepotCoordinate as 库位坐标,Remark as  备注 from Depot  order by  cast(DepotName as numeric) ";
                DataTable pDTMain = pObj_Comm.ExeForDtl(ps_Sql);
        ...
        public DataTable GetDepotData3(string depotpara)
        {

            CommonInterface pObj_Comm = CommonFactory.CreateInstance(CommonData.sql);
            try
            {
                string ps_Sql = "select DepotGuid,DepotName as 库位,DepotPerson as  负责人,DepotCoordinate as 库位坐标 from Depot where DepotCoordinate='" + depotpara + "'"  ;
                DataTable pDTMain = pObj_Comm.ExeForDtl(ps_Sql);
